...
`src/` contains the plugin code. Main entry points are `src/index.ts`, `src/main.ts`, and `src/App.vue`. Core search/replace logic lives in `src/features/search-replace/`, which is now split into `store/` for search session orchestration, `attribute-view/` for attribute-view discovery and value normalization, `editor/` for DOM adapters/highlighting/replacement helpers, and `ui/` for panel composables such as the minimap. Shared UI primitives live in `src/components/SiyuanTheme/`, locale files live in `src/i18n/`, and cross-cutting settings/hotkey helpers live at the top of `src/`. Tests live in `tests/` with SiYuan mocks in `tests/mocks/`. Project docs are in `docs/`; check `docs/project-structure.md` for the current module map and `docs/refactor-plan.md` for the refactor execution record. `reference_docs/` stores SiYuan plugin development references, `plugin-sample-vite-vue/` is the development template, release checks live in `scripts/`, and static assets live in `assets/`, `icon.png`, and `preview.png`. Build output goes to `dist/` and `package.zip`.
