...
describe('search panel i18n', () => {
  ...
  it('renders the panel in English when the plugin locale is English', async () => {
    ...
    expect((host?.querySelector('.sfsr-count__input') as HTMLInputElement)?.value).toBe('1')
    expect(host?.querySelector('.sfsr-count__total')?.textContent?.trim()).toBe('/ 1')
    expect(host?.querySelector('button[title="Replace"]')).not.toBeNull()
    expect(host?.querySelector('button[title="Skip"]')).not.toBeNull()
    expect(host?.querySelector('button[title="Replace all"]')).not.toBeNull()
